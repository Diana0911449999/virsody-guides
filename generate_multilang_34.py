...
POSTER_SOURCE = ROOT / "poster-34-source.jpg"
VOICES = {code: helper.VOICES[code] for code in ("zh-TW", "en-US", "ja-JP")}


def load_languages():
    html = (ROOT / "34-formal.html").read_text(encoding="utf-8")
...
def main():
    ...
    for code, item in languages.items():
        ...
        text_path = ROOT / f"narration-34-{code}.txt"
        audio_path = OUT / f"narration-34-{code}.mp3"
        srt_path = ROOT / f"narration-34-{code}.srt"
    ...
    (OUT / "languages-34.js").write_text(
        "window.GUIDE_LANGUAGES=" + json.dumps(languages, ensure_ascii=False, separators=(",", ":")) + ";\n",
        encoding="utf-8")
    shutil.copy2(POSTER_SOURCE, OUT / "poster-34.jpg")
    html = (OUT / "33.html").read_text(encoding="utf-8")
    ...
    html = re.sub(r'(<figure class="poster"><img src=")[^"]+(" alt=")[^"]+(">)',
                  rf'\1poster-34.jpg?v=1\2{languages["zh-TW"]["title"]}\3', html, count=1)
    html = re.sub(r'(<p id="eyebrow" class="eyebrow">).*?(</p>)', r'\1GUIDE 34\2', html, count=1)
    ...
    html = re.sub(r'languages-33\.js\?v=\d+', "languages-34.js?v=1", html, count=1)
    html = re.sub(r'narration-33-zh-TW\.mp3\?v=\d+', "narration-34-zh-TW.mp3?v=1", html, count=1)
    ...
    (OUT / "34.html").write_text(html, encoding="utf-8")
    print(f"Generated 34.html and {len(languages)} audio files; Chinese duration {duration:.1f}s")
