...
POSTER_SOURCE = ROOT / "poster-36-source.jpg"
VOICES = {code: helper.VOICES[code] for code in ("zh-TW", "en-US", "ja-JP")}


def load_languages():
    html = (ROOT / "36-formal.html").read_text(encoding="utf-8")
...
def main():
    ...
    for code, item in languages.items():
        ...
        text_path = ROOT / f"narration-36-{code}.txt"
        audio_path = OUT / f"narration-36-{code}.mp3"
        srt_path = ROOT / f"narration-36-{code}.srt"
    ...
    (OUT / "languages-36.js").write_text(
        ...
        encoding="utf-8")
    shutil.copy2(POSTER_SOURCE, OUT / "poster-36.jpg")
    html = (OUT / "35.html").read_text(encoding="utf-8")
    ...
    html = re.sub(r'(<p id="eyebrow" class="eyebrow">).*?(</p>)', r'\1GUIDE 36\2', html, count=1)
    html = re.sub(
        r'(<h1 id="title">).*?(</h1>)',
    ...
    (OUT / "36.html").write_text(html, encoding="utf-8")
    print(f"Generated 36.html and {len(languages)} audio files; Chinese duration {duration:.1f}s")
