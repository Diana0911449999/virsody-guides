...
POSTER_SOURCE = ROOT / "poster-37-source.jpg"
VOICES = {code: helper.VOICES[code] for code in ("zh-TW", "en-US", "ja-JP")}


def load_languages():
    html = (ROOT / "37-formal.html").read_text(encoding="utf-8")
...
def main():
    ...
    for code, item in languages.items():
        ...
        text_path = ROOT / f"narration-37-{code}.txt"
        audio_path = OUT / f"narration-37-{code}.mp3"
        srt_path = ROOT / f"narration-37-{code}.srt"
    ...
    (OUT / "languages-37.js").write_text(
        "window.GUIDE_LANGUAGES=" + json.dumps(languages, ensure_ascii=False, separators=(",", ":")) + ";\n",
        encoding="utf-8")
    shutil.copy2(POSTER_SOURCE, OUT / "poster-37.jpg")
    html = (OUT / "36.html").read_text(encoding="utf-8")
    html = re.sub(r"<title>.*?</title>", f"<title>{languages['zh-TW']['title']}｜Audio Guide</title>", html, count=1)
    html = re.sub(
        r'(<figure class="poster"><img src=")[^"]+(" alt=")[^"]+(">)',
        lambda match: match.group(1) + "poster-37.jpg?v=1" + match.group(2)
        + languages["zh-TW"]["title"] + match.group(3),
        html,
        count=1,
    )
    html = re.sub(r'(<p id="eyebrow" class="eyebrow">).*?(</p>)', r'\1GUIDE 37\2', html, count=1)
    html = re.sub(
        r'(<h1 id="title">).*?(</h1>)',
    ...
    html = re.sub(r'languages-36\.js\?v=\d+', "languages-37.js?v=1", html, count=1)
    html = re.sub(r'narration-36-zh-TW\.mp3\?v=\d+', "narration-37-zh-TW.mp3?v=1", html, count=1)
    ...
    (OUT / "37.html").write_text(html, encoding="utf-8")
    print(f"Generated 37.html and {len(languages)} audio files; Chinese duration {duration:.1f}s")
