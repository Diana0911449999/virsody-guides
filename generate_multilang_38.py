...
POSTER_SOURCE = ROOT / "poster-38-source.jpg"
VOICES = {code: helper.VOICES[code] for code in ("zh-TW", "en-US", "ja-JP")}


def load_languages():
    html = (ROOT / "38-formal.html").read_text(encoding="utf-8")
...
def main():
    ...
    for code, item in languages.items():
        ...
        text_path = ROOT / f"narration-38-{code}.txt"
        audio_path = OUT / f"narration-38-{code}.mp3"
        srt_path = ROOT / f"narration-38-{code}.srt"
    ...
    (OUT / "languages-38.js").write_text(
        "window.GUIDE_LANGUAGES=" + json.dumps(languages, ensure_ascii=False, separators=(",", ":")) + ";\n",
        encoding="utf-8")
    shutil.copy2(POSTER_SOURCE, OUT / "poster-38.jpg")
    html = (OUT / "37.html").read_text(encoding="utf-8")
    html = re.sub(r"<title>.*?</title>", f"<title>{languages['zh-TW']['title']}｜Audio Guide</title>", html, count=1)
    html = re.sub(
        r'(<figure class="poster"><img src=")[^"]+(" alt=")[^"]+(">)',
        lambda match: match.group(1) + "poster-38.jpg?v=1" + match.group(2)
        + languages["zh-TW"]["title"] + match.group(3),
        html,
        count=1,
    )
    html = re.sub(r'(<p id="eyebrow" class="eyebrow">).*?(</p>)', r'\1GUIDE 38\2', html, count=1)
    html = re.sub(
        r'(<h1 id="title">).*?(</h1>)',
    ...
    html = re.sub(r'languages-37\.js\?v=\d+', "languages-38.js?v=1", html, count=1)
    html = re.sub(r'narration-37-zh-TW\.mp3\?v=\d+', "narration-38-zh-TW.mp3?v=1", html, count=1)
    ...
    (OUT / "38.html").write_text(html, encoding="utf-8")
    print(f"Generated 38.html and {len(languages)} audio files; Chinese duration {duration:.1f}s")
